...
export class SessionIndexStore {
  ...
  async create(session: SessionEntry): Promise<void> {
    await this.db
      .prepare(
        `INSERT OR IGNORE INTO sessions (id, title, repo_owner, repo_name, model, reasoning_effort, base_branch, branch_name, status, parent_session_id, spawn_source, creation_source, spawn_depth, automation_id, automation_run_id, scm_login, user_id, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      )
      .bind(
        session.id,
