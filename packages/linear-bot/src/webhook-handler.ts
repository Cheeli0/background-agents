...
async function handleNewSession(
  ...
  const teamId = issueDetails?.team.id ?? issue.team?.id ?? null;

  if (webhook.action === "created" && issueDetails?.state && teamId) {
    ...
    if (startedTransition.status === "updated") {
      log.info("agent_session.issue_moved_to_started", {
        ...
        issue_id: issue.id,
        issue_identifier: issue.identifier,
        previous_state_type: issueDetails.state.type,
      ...
      log.warn("agent_session.issue_move_to_started_failed", {
        ...
        issue_id: issue.id,
        issue_identifier: issue.identifier,
        current_state_type: issueDetails.state.type,
