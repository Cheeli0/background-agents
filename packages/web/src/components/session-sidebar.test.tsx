...
describe("SessionSidebar", () => {
  ...
  it("auto-archives overflow sessions for merged or closed PRs", async () => {
    const sessions = Array.from({ length: 12 }, (_, index) => createSession(12 - index));
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url === "/api/sessions/session-2/archive" || url === "/api/sessions/session-1/archive") {
        expect(init?.method).toBe("POST");
        return new Response(null, { status: 200 });
      }
    ...
    await waitFor(() => {
      expect(fetchMock).toHaveBeenCalledWith("/api/sessions/session-2/archive", { method: "POST" });
      expect(fetchMock).toHaveBeenCalledWith("/api/sessions/session-1/archive", { method: "POST" });
    });
  ...
  it("keeps overflow sessions visible when their PRs are still open", async () => {
    const sessions = Array.from({ length: 12 }, (_, index) => createSession(12 - index));

    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      const url = String(input);

      if (url.endsWith("/session-2/associated-pr") || url.endsWith("/session-1/associated-pr")) {
        return jsonResponse({ pullRequest: { status: "open" } });
      }
    ...
    await waitFor(() => {
      expect(fetchMock).toHaveBeenCalledWith("/api/sessions/session-2/associated-pr");
      expect(fetchMock).toHaveBeenCalledWith("/api/sessions/session-1/associated-pr");
    });

    expect(fetchMock).not.toHaveBeenCalledWith("/api/sessions/session-1/archive", {
      method: "POST",
    });
    expect(fetchMock).not.toHaveBeenCalledWith("/api/sessions/session-2/archive", {
      method: "POST",
    });
  });

  it("respects the session panel auto-archive preference", async () => {
    localStorage.setItem(
      "session-panel-preferences",
