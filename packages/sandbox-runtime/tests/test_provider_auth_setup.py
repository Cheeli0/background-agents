...
def test_setup_provider_auth_writes_selected_provider_entries(tmp_path):
    server = make_server(tmp_path, "opencode-go", "glm-5.1")

    with (
        patch.dict("os.environ", {"OPENCODE_GO_API_KEY": "secret"}, clear=True),
        patch("sandbox_runtime.opencode_server.write_auth_entries") as write_entries,
    ):
        server._setup_provider_auth()

    write_entries.assert_called_once_with({"opencode-go": {"type": "api", "key": "secret"}})
